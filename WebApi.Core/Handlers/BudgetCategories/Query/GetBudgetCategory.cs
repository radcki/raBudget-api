...
namespace raBudget.Core.Handlers.BudgetCategories.Query
{
    public class GetBudgetCategory
    {
        ...
        public class Handler : BaseBudgetCategoryHandler<Query, BudgetCategoryDto>
        {
            ...
            {
                var isAccessible = await BudgetCategoryRepository.IsAccessibleToUser(AuthenticationProvider.User.UserId, query.BudgetCategoryId);
                if (!isAccessible)
                {
